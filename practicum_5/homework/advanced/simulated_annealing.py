...
def solve_via_simulated_annealing(
    ...
    loss_history = np.zeros((n_iters,), dtype=np.int_) # Список для хранения значений потерь
    current_colors = initial_colors # Текущая раскраска графа
    current_loss = number_of_conflicts(G, current_colors) # Текущее количество конфликтов
    t_start = 100 # Начальная температура для имитации отжига
    t_min = 0.0001 # Минимальная температура
    k = 0.1 # Коэффициент охлаждения температуры

    for i in range(n_iters):
        t_start = max(t_start * k, t_min)
        new_colors = tweak(current_colors, n_max_colors) # Генерация новой раскраски
